! ==============================================================================
! File:        mixed_double_corrector.f90
...
!              equations for the Hamiltonian dynamics of the mixed (p+s)-spin model
!              In the context of my Master's thesis, this version
!              is specialized to the case p = 2 and s = 3.
!
!              Because we are interested to probe the integrablòe dynamics (p=2) with
!              with a non-integrable term, we wheight each contribution with some 
!              coefficients: c2, c3, c2_zero, c3_zero. Setting them all equals to one
!              correspond to study the stationary dynamics of the mixed model. 
!              Moreover, if re are interested to probe the integrable dinamics with a 
!              non integrable term, whe have to change the value of those coefficients.
!
...
!              corrector scheme. The dynamics is contraint to a sphere: this
!              constraint is codified by the evolution of the lagrange multiplier.
! 
!              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!              THIS CODE IS SPECIALIZED TO THE HIGH TEMPERATURE REGIME
!              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
...
! Compilation:  
!             - For small matrices:
!                  gfortran -g -o mixed_double_corrector mixed_double_corrector.f90
!             - For large matrices (better RAM handling):
!                  gfortran -g -mcmodel=medium -fPIC -o mixed_double_corrector mixed_double_corrector.f90
!
! Usage:   ./mixed_ double_corrector mass diss itemp Jf
!
! Notes:
...
program matrices
  ...
  ! weight constats
  double precision c3,c3_zero,c2,c2_zero
  ...
  !mass=1.d0
  J0=1.d0
  ! changing the values of the following constants, we describes differents dynamics
  ! - if we set c2=c3=c3_zero=c2_zero         we describe the hamiltonian dynamics of the mixed model
  ! - if we set c2=c3=c3_zero and c2_zero=0   we start from a p3 inital state and we evolve with the mixed model
  ! - if we set c2=c3=c2_zero and c3_zero=0   we start from a p2 inital state and we evolve with the mixed model
  ! - if we set c2<<c3 and c3_zero<<c2_zero    we want to undesrand how a non itegrable pertrurbation can change the itegrable dynamics
  ! the are other possibilities that can be studied.
  ! differently, time-dependent coefficints need a different code and theoretical adjustments
  c3=1.d0
  c3_zero=1.d0
  c2=1.d0
  c2_zero=1.d0
  zf=itemp + c2*c2_zero*(J0*p)/(2*itemp) + c3_zero*c3*(J0*s)/(2*itemp)
  ...
  do i=1,n
    ...
    I_mu_pred=0.d0
    do k=1,i
          !I1_mu_pred=(J0**2)*pi_c_pred(i,k)*(3.d0*c3*c3*c_pred(i,k) + 1*c2*c2)*r_pred(i,k)
          !I2_mu_pred=(J0**2)*((3.d0*c3*c3*c_pred(i,k) + 1*c2*c2)*pi_r_pred(i,k) + 3.d0*c3*c3*pi_c_pred(i,k) * r_pred(i,k))*c_pred(i,k)
          !I3_mu_pred=3.d0*(J0**2)*r_pred(i,k)*(c2*c2*(c_pred(i,k))**(p-2) + 3.d0*c3*c3*(c_pred(i,k))**(s-2))*pi_c_pred(i,k)
          !I4_mu_pred=3.d0*(J0**2)*(c2*c2*p*0.5d0*(c_pred(i,k))**(pminus) + c3*c3*s*0.5d0*(c_pred(i,k))**(sminus))*pi_r_pred(i,k)
          I1_mu_pred = (pi_r_pred(i,k)*(c_pred(i,k)))*(15*0.5d0*c3*c3*(c_pred(i,k))+4*c2*c2)
          I2_mu_pred = (pi_c_pred(i,k)*(r_pred(i,k)))*(15*c3*c3*(c_pred(i,k))+4*c2*c2)
          I_mu_pred= I_mu_pred + (I1_mu_pred + I2_mu_pred)
    enddo
    I_mu_pred = I_mu_pred*h
    ci(i) = (1/itemp)*(pi_c_pred(i,1)*(c_pred(i,1)))*(15*0.5d0*c3*c3_zero*(c_pred(i,1))+4*c2*c2_zero)
    ...
    do j=1,i
      ...
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%      
      !!! I1 = int_{t'}^{t}dt'' Sigma(t,t'')^(p-2)r(t'',t')
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      I1_pred = 0.d0
      do k=j,i
        I1_pred = I1_pred + (J0**2)*c2*c2*r_pred(i,k)*r_pred(k,j) + c3*c3*(J0**2)*3.d0*r_pred(i,k)*((c_pred(i,k))**(s-2))*r_pred(k,j)
      enddo
      I1_pred = I1_pred*h
      F1_pred(j) = I1_pred
      ...
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      !!!  I3 = int_0^t' dt'' D(t,t'') r(t',t'')
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      I3_pred = 0.d0
      do k =1,j
        I3_pred=I3_pred+(J0**2)*(c2*c2*(c_pred(i,k))+ c3*c3*3*0.5*(c_pred(i,k)**(2)))*r_pred(j,k)
      enddo
      I3_pred = h*I3_pred

      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      !!! I5 = int_0^t dt''Sigma(t,t'')*c(t'',t')
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      I5_pred = 0.d0
      do k=1,i              
        if(k.le.j) then
          I5_pred=I5_pred+(J0**2)*r_pred(i,k)*(c2*c2 + c3*c3*3.d0*(c_pred(i,k)))*c_pred(j,k)  ! the reason of this if is explained in the pure codes
        else                                                                                  ! in our code is unnecessary becouse we works with the 
          I5_pred=I5_pred+(J0**2)*r_pred(i,k)*(c2*c2 + c3*c3*3.d0*(c_pred(i,k)))*c_pred(k,j)  ! entire c_pred matrix. If we would vectorize it (it is possible)
        endif                                                                                 ! you should take into accout it in orde to perform causal integrals in the right way  
      enddo
      I5_pred = h*I5_pred
      F2_pred(j)= I3_pred + I5_pred + (((J0**2)/itemp)*c2*c2_zero*(c_pred(i,1)**(pminus)) + ((0.5*s*c3*c3_zero*(J0**2))/itemp)*(c_pred(i,1)**(sminus)))*c_pred(j,1) ! the last term is linked to the  
                                                                                                                                                                    ! contribution of the intial state
      ! updating the evolution                                                                                                                                                             ! configuration
      if (i==j) then
    ...
    I_mu_corr=0.d0
    do k=1,i+1
          I1_mu_corr = (pi_r_pred(i+1,k)*(c_pred(i+1,k)))*(15*0.5d0*c3*c3*(c_pred(i+1,k))+(4.d0*c2*c2))
          I2_mu_corr = (pi_c_pred(i+1,k)*(r_pred(i+1,k)))*(15*c3*c3*(c_pred(i+1,k))+(4.d0*c2*c2))
          I_mu_corr= I_mu_corr + (I1_mu_corr + I2_mu_corr)
    enddo
    I_mu_corr = I_mu_corr*h
    ci_corr(i) = (1/itemp)*(pi_c_pred(i+1,1)*(c_pred(i+1,1)))*(15d0*0.5d0*c3*c3_zero*(c_pred(i+1,1))+4*c2*c2_zero)
    ...
    do j=1,i
      ...
      I1_corr = 0.d0
      do k=j,i+1
        I1_corr = I1_corr + c2*c2*(J0**2)*r_pred(i+1,k)*((c_pred(i+1,k))**(p-2))*r_pred(k,j) + 3.d0*c3*c3**(J0**2)*r_pred(i+1,k)*((c_pred(i+1,k))**(s-2))*r_pred(k,j)
      enddo
      I1_corr = I1_corr*h
      F1_corr(j) = I1_corr
      ...
      I3_corr = 0.d0
      do k =1,j
      I3_corr=I3_corr+(p*(J0**2)*0.5*(c2*c2*c_pred(i+1,k)**(pminus)) + c3*c3*s*(J0**2)*0.5*(c_pred(i+1,k)**(sminus)))*r_pred(j,k)
      enddo
      I3_corr = h*I3_corr
      ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      I5_corr = 0.d0
      do k=1,i
        if (k.le.j) then
          I5_corr=I5_corr+r_pred(i+1,k)*(c2*c2*J0*(c_pred(i+1,k)**(p-2)) + c3*c3*s*J0*(c_pred(i+1,k)**(s-2)) )*c_pred(j,k)
        else
          I5_corr=I5_corr+r_pred(i+1,k)*(c2*c2*J0*(c_pred(i+1,k)**(p-2)) + c3*c3*s*J0*(c_pred(i+1,k)**(s-2)) )*c_pred(k,j)
        endif
      enddo
      I5_corr = h*I5_corr
      F2_corr(j)= I3_corr + I5_corr + (c2_zero*c2*((0.5*p*(J0**2))/itemp)*(c_pred(i+1,1)**(pminus))+ (c3*c3_zero*(0.5*s*(J0**2))/itemp)*(c_pred(i+1,1)**(sminus)))*c_pred(j,1) 
