! ==============================================================================
! File:        p2_central_derivative_no_corr.f90
...
!              equations for the Hamiltonian dynamics of the p = 2 spin model.
!
!              Unlike the p ≥ 3 case handled with a Predictor–Corrector algorithm,
!              this version uses a central-difference integration scheme to address
!              numerical instabilities arising at high temperatures when using PC.
!
!              No correction step is required here, as the convergence is naturally
!              achieved without additional iterations.
!
!              The model evolves under conservative dynamics with spherical
!              constraints, and computes both the correlation and response functions
...
! Compilation:  
!             - For small matrices:
!                  gfortran -g -o p2_central_derivative p2_central_derivative_no_corr.f90
!             - For large matrices (better RAM handling):
!                  gfortran -g -mcmodel=medium -fPIC -o p2_central_derivative p2_central_derivative_no_corr.f90
!
! Usage:   ./p2_central_derivative mass diss itemp Jf
!
! Notes:
...
!   - This version does not rely on the Correale formulation used for p = 3.
!
! For theoretical motivations and full implementation details, please refer
...
program matrices
  ...
  ! Respect to the PC algoritm, we do not declare anycorrection 
  ! variable becasue we do not use any corrector step
  ! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  real(8), dimension(n+1, n+1) :: r_pred, c_pred, pi_c_pred, pi_r_pred, q_pred,pi_q_pred,F1_pred,F2_pred
  real(8), dimension(n+1)    :: mu_pred,ci,F3_pred,F3 ! Vector mu_pred
  double precision plot_r(0:ntot),plot_c(0:ntot)
  double precision I_mu_pred_vec(0:ntot), F4_pred(0:ntot)
  ! loop varibales
  integer :: i,j,k,l
  ! integer varibales
  integer :: p,pminus
  ...
  !
  ! We create and we open each of the imput file
  !
  ...
  ! We define the initial value of the Lagrange multiplier and the inital overlap q_in. 
  if(p==2) then
    if (itemp .ge. J0) then
      q_in = 0.d0
      zf = itemp + (J0**2)/(itemp)
  ...
  ! respect to the Predicto corrector scheme, here we need more point in order to evolve the
  ! dynamics correctil. So, we have expanded at Firts order the Green's function and we have 
  ! those intial conditions
  pi_r_pred(1,1) = 1.d0/mass
  r_pred(1,1) = h/mass
  pi_c_pred(1,1) = -h*itemp/mass
  c_pred(1,1) = 1-(h**2)*itemp/mass
  !print*, q_pred(1,1)
  mu_pred(1)=z_in
